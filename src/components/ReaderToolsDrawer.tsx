...
import {
  ...
  EyeOff, GraduationCap, ZoomIn, ZoomOut
} from "lucide-react";
  ...
  const tools = [
    ...
    [
      ...
      },
      {
        ...
        label: "انتقال لصفحة",
        onClick: () => { onTogglePageNav(); onOpenChange(false); },
      },
      {
        icon: <List className="size-5" />,
        label: "الفهرس",
        onClick: () => { onToggleJuzIndex(); onOpenChange(false); },
      },
    ],
    ...
    [
      ...
      },
      {
        ...
        label: "مصدر الصور",
        onClick: () => { onToggleSourceSelector(); onOpenChange(false); },
      },
      {
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        >
          ...
          <div className="flex flex-col gap-4 pb-4" dir="rtl">
                ...
                {row.map((tool, toolIdx) => {
                  ...
                  if ('isShare' in tool && tool.isShare) {
                    return (
                      <div
                        key={toolIdx}
                        className="flex flex-col items-center justify-center gap-1.5 p-3 rounded-2xl bg-muted/30 hover:bg-muted/60 transition-all active:scale-95 cursor-pointer"
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex flex-col items-center justify-center gap-1.5 p-3 rounded-2xl transition-all active:scale-95",
                        'active' in tool && tool.active
                          ? "bg-accent/15 text-accent border border-accent/20"
                          : "bg-muted/30 hover:bg-muted/60 text-primary/80",
            ...
            <div className="flex items-center justify-center gap-3 p-3 rounded-2xl bg-muted/30">
              <button
                onClick={onZoomOut}
