...
export const AudioUnlockBanner = () => {
  ...
  return (
    <div
      className="fixed bottom-24 left-4 right-4 z-[100] md:left-auto md:right-4 md:w-96 transition-all duration-500 opacity-100 translate-y-0"
    >
      <div className="bg-amber-500 text-white p-4 rounded-2xl shadow-2xl flex items-center gap-4 border border-white/20">
        ...
        <div className="flex flex-col gap-2">
          <button
            ...
            className="bg-white text-amber-600 px-4 py-2 rounded-xl font-naskh text-xs font-bold hover:bg-white/90 transition-all shadow-md whitespace-nowrap active:scale-95"
          >
            {isAr ? "تفعيل الآن" : "Enable Now"}
          </button>
          <button
            onClick={() => setIsVisible(false)}
            className="text-white/60 hover:text-white transition-colors flex items-center justify-center active:scale-90"
            aria-label="Close"
          >
            <X size={16} />
          </button>
