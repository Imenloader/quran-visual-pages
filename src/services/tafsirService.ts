...
export async function fetchTafsir(surah: number, ayah: number, signal?: AbortSignal): Promise<TafsirResponse> {
  ...
  // Source 1: Quran.com API (Primary - Tafsir Al-Muyassar Arabic)
  try {
    const data = await fetchWithCache(`https://api.quran.com/api/v4/tafsirs/16/by_ayah/${verseKey}`, { signal, timeout: 8000 });
