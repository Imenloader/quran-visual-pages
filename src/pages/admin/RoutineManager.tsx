      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div className="text-right">
            ...
            <p className="text-muted-foreground mt-1 text-right">تحديد المهام اليومية الصباحية والمسائية</p>
          </div>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4">
          {tasks.map((task) => (
            <div 
              key={task.id} 
              className="bento-card !p-4 flex flex-wrap md:flex-nowrap gap-4 items-center transition-all hover:shadow-md"
            >
              <Button variant="ghost" size="icon" onClick={() => removeTask(task.id)} className="text-rose-500 hover:bg-rose-500/10 shrink-0">
                <Trash2 size={16} />
              </Button>
              <Input 
                value={task.titleEn} 
              ...
              <Input 
                ...
                className="rounded-xl h-10 flex-1 min-w-[200px] text-right"
              />
              <select 
                value={task.category} 
