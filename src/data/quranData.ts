...
export const surahByNumber = new Map<number | string, SurahInfo>(
  surahData.map((s) => [s.number, s])
);
// Also add string keys to surahByNumber for convenience when parsing keys
surahData.forEach(s => surahByNumber.set(s.number.toString(), s));
